...
import * as Pswd from "../artifacts/contracts/Pswd.sol/Pswd.json";
import { DOC_ID_KEY, PSWD_CONTRACT_ADDRESS } from "../constants";
...
export default function Home() {
  ...
  let saveCredentials = async (e) => {
    ...
    if (!(domainname && username && passwordRef.current.value)) {
    ...
    if (token !== "0") {
      if (docId === null) {
        docId = prompt(
          "Doc ID is not found on local storage please enter the doc id or else a new docid will be created and the old data is lost encrypted"
        );
        if (docId) {
          ...
          localStorage.setItem(DOC_ID_KEY, docId);
          alert(
            `Doc ID saved into local storage of this browser, please save this doc id ${docId}`
          );
        }
      } else {
